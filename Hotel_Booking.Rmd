...
df
# month_vector <- c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November",  "December")
...
```{r Making Categorical Variables}
# assigned_room_type, reserved_room_type, hotel, meal, country, distribution_channel, market_segment, deposit type, customer_type
# assignned room type is ABCDEFGHIKL
# reserved_room_type is ABCDEFGHL
...
# looks like there's a peak around July each year

# df_month <- df[,, by= .(arrival_date_month, hotel)]
# df_month
# ggplot(df_month, aes(x = arrival_date_month, y = sum(is_canceled), group = hotel)) + geom_line(aes(color = arrival_date_year)) 

```

```{r Correlation of numerical features}
